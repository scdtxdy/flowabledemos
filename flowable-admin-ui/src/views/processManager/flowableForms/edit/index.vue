  ...
  <div class="form-making-height">
    <fm-making-form ref="makingFrom" preview generate-json>
      <template slot="action">
        <el-button type="text" icon="el-icon-upload" @click="btnSave">保存
        </el-button>
      </template>
    </fm-making-form>
  </div>
  ...
  export default {
    ...
    methods: {
      getFormData() {
        ...
        getAction('/vue-admin-template/rest/form/queryById', {id: this.formKey}).then(({data}) => {
          this.formData = data
      ...
      btnSave() {
        ...
        putAction('/vue-admin-template/rest/form/update', this.formData).then(({msg, data}) => {
          Message.success(msg)
        })
      }
    }
