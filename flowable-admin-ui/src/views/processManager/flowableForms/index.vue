...
<script>
  ...
  export default {
    ...
    methods: {
      list() {
        getAction('/vue-admin-template/rest/form/list', this.listQuery).then(res => {
          const {data} = res
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            postAction('/vue-admin-template/rest/form/save', this.temp).then(({msg}) => {
      ...
      updateData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            putAction('/vue-admin-template/rest/form/update', this.temp).then(({msg}) => {
              this.dialogFormVisible = false
              Message.success(msg)
              this.list()
            })
      ...
      btnDelete(id) {
        ...
        deleteAction('/vue-admin-template/rest/form/delete', {ids: ids.toString()}).then(({msg}) => {
          Message.success(msg)
          this.list()
        })
      },
